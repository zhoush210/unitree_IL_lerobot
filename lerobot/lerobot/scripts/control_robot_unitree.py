...
def record(
    ...
    if not is_headless():
        ...
        def on_press(key):
            nonlocal exit_early, rerecord_episode, stop_recording, start_recording
            try:
                if key == keyboard.Key.right:     # stop 
                    print("Right arrow key pressed. Exiting loop...")
                    exit_early = True
                # elif key == keyboard.Key.left:      # rerecord
                ...
                #     exit_early = True
                    ...
                    stop_recording = True
                    exit_early = True
                elif key == keyboard.Key.up:       # exit 
                    print("up key pressed. start data recording...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_image_writers) as executor:
        ...
        while episode_index < num_episodes:
            ...
            while timestamp < episode_time_s and start_recording:
                ...
                for key in not_image_keys:
                    if key not in ep_dict:
                        ep_dict[key] = []
                    ep_dict[key].append(observation[key].squeeze())

                for key in action:
                    if key not in ep_dict:
                        ep_dict[key] = []
                    ep_dict[key].append(action[key].squeeze())
    ...
    for episode_index in tqdm.tqdm(range(num_episodes)):
        for key in image_keys:
            ...
            encode_video_frames(tmp_imgs_dir, video_path, fps, overwrite=True, vcodec='libx264')
...
if __name__ == "__main__":
    ...
    print("robot_overrides", robot_overrides)
    print("kwargs", kwargs)
